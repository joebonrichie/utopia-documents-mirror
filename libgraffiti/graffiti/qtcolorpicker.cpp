...
#include <graffiti/qtcolorpicker_p.h>
...
void QtColorPicker::setLabels(bool labels)
{
    this->_labels = labels;
}

bool QtColorPicker::labels()
{
    return this->_labels;
}
...
{
    ColorPickerPopup popup(NULL, -1, allowCustomColors);
...
QColor QtColorPicker::getColorFromPopup(const QPoint &point, bool /* allowCustomColours */)
{
    this->popup->move(point);
    this->popup->exec();
    return this->popup->lastSelected();
}
...
*/
ColorPickerPopup::ColorPickerPopup(QtColorPicker *picker, int width, bool withColorDialog,
                                   QWidget *parent)
    : QFrame(parent, Qt::Popup), _picker(picker)
{
...
QtColorPicker *ColorPickerPopup::picker()
{
    return this->_picker;
}
...
*/
ColorPickerItem::ColorPickerItem(ColorPickerPopup *popup, const QColor &color, const QString &text,
                                 QWidget *parent)
    : QFrame(parent), c(color), t(text), sel(false), _popup(popup)
{
    setToolTip(t);
    setFixedWidth(24);
    setFixedHeight(21);
...
ColorPickerPopup *ColorPickerItem::popup()
{
    return this->_popup;
}
...
void ColorPickerItem::paintEvent(QPaintEvent *)
{
    ...
    if (this->popup()->picker()->labels() == false)
    {
        p.fillRect(QRect(4, 4, w - 8, h - 8), QBrush(c));
    }
    else
    {
        p.fillRect(QRect(4, 4, w - 8, h - 8), Qt::gray);
    }


    if (this->popup()->picker()->labels())
    {
        p.drawText(QRect(4, 4, w - 8, h - 8),Qt::AlignCenter, t.left(1));
    }
