...
namespace Utopia
{
    ...
    UpdateInitializer::UpdateInitializer()
        : Initializer()
    {
        updateWidget = new UpdateWidget();
    }

    void UpdateInitializer::init()
    {
        this->setMessage("Checking for updates...");

        // Check for updates and display update dialog if any available
        updateWidget->check();
    }

    void UpdateInitializer::final()
    {
        delete updateWidget;
    }

    QString UpdateInitializer::description()
    {
...
{
    UTOPIA_REGISTER_EXTENSION(Utopia::UpdateInitializer);
}
