...
def fetch(pmid):
    return common.eutils.efetch(id=pmid)
...
def resolve(id, field):
    results = common.eutils.esearch(term='"{0}"[{1}]'.format(id, field.upper()), retmax=1)
...
_PubmedArticle_fields = {
    ...
    'MedlineCitation/Article[1]/Journal/ISSN[1]/text()': 'issn',
...
def parse_PubmedArticle(article):
    ...
    for path, key in _PubmedArticle_fields.iteritems():
        ...
        identifiers = {}
        if 'issn' in citation:
            identifiers['issn'] = citation['issn']
            del citation['issn']
