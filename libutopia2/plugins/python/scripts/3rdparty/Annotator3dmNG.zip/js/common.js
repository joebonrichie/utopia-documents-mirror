...
function getUniqueMutationsForEquivalentTable() {
	...
	for (var proteinId in equivalentMutationsByProteinId) {
		for (var i = 0; i < equivalentMutationsByProteinId[proteinId].length; i++) {
			...
			table += "<tr><td class=\"proteinId\"><a href=\"https://" + webhost + "/index.php?&amp;mode=pdetail&amp;proteinName=" + proteinAc + "&amp;familyid=1&amp;filterid=1&amp;numberingscheme=-1&amp;sfamid=" + domainId  + "\">"+ proteinId + "</a></td><td>" + mutationString[0] + "</td><td>" + mutationString.slice(1,-1) + "</td><td>" + mutationString.slice(-1) + "</td><td><a href=\"" + pubmedUrl + "\">"  + "Pubmed (" + getUniquePubmedIds(mutations).length + "</a>)</td></tr>"; 
...
function getUniqueMutationsForPositionTable(residueNumber) {
	...
	if (mutationList.length == 0) {
		...
		for (var i = 0; i < mutationList.length; i++) {
			...
			var toType = mutation.toType;
			var pubmedUrl = getPubmedUrl(mutations);
			table += "<tr><td>" + mutationString[0] + "</td><td>" + mutationString.slice(1,-1) + "</td><td>" + mutationString.slice(-1) + "</td><td><a href=\"" + pubmedUrl + "\">"  + "Pubmed (" + getUniquePubmedIds(mutations).length + "</a>)</td></tr>"; 
		}
		table += "</tbody></table>";
		return table;
	}
		...
		changestart: function(event, ui) {
			...
			if (ui.newHeader.attr('data-url') == "mutationsInProteinUrl") {
				if ($('.mutationsInProtein').html().length == 0) {
					$('.mutationsInProtein').html("<div align=\"center\"><img src=\"https://3dm.bio-prodict.nl/utopia/ajax-loader.gif\"/></div>");
					console.log(url)
			...
			if (ui.newHeader.attr('data-url') == "mutationsAtEquivalentPositionsUrl") {
				if ($('.mutationsAtEquivalentPositions').html().length == 0) {
					$('.mutationsAtEquivalentPositions').html("<div align=\"center\"><img src=\"https://3dm.bio-prodict.nl/utopia/ajax-loader.gif\"/></div>");
