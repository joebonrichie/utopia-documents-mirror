...
#include <athenaeum/libraryview.h>
#include <athenaeum/libraryview_p.h>
...
namespace Athenaeum
{
    ...
    void LibraryStatusIconPrivate::onStateChanged(Athenaeum::AbstractBibliographicCollection::State state)
    {
        spinner->setVisible(state == AbstractBibliographicCollection::BusyState);
        ...
        : QWidget(), d(new LibraryStatusIconPrivate(this))
    {
        ...
        if (QAbstractItemModel * qaim = model->index(row, 0, parentIndex).data(LibraryModel::ModelRole).value< QAbstractItemModel * >()) {
            connect(qaim, SIGNAL(stateChanged(Athenaeum::AbstractBibliographicCollection::State)),
                    d, SLOT(onStateChanged(Athenaeum::AbstractBibliographicCollection::State)));
            // Initial spinner state
            if (AbstractBibliographicCollection * collection = qobject_cast< AbstractBibliographicCollection * >(qaim)) {
                ...
                if (index >= 0) {
                    ...
                    case QEvent::DragMove: {
                        ...
                        if (index.isValid()) {
                            ...
                            //drop.row = view->model()->rowCount();
                            drop.row = -1;
                        }
                        //qDebug() << "==== MOVE" << drop.index << drop.row;
