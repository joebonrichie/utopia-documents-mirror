...
#ifndef GTL_MATRIX2_INCL_
#define GTL_MATRIX2_INCL_

// Cannot be included before <gtl/matrix.h>
#ifndef GTL_MATRIX_INCL_
#error Do not include <gtl/bits/matrix2.h> directly; include <gtl/matrix.h> instead.
#endif
...
namespace gtl
{
    ...
    class matrix< _ComponentType, 2 >
    {
        // Convenience typedefs
        typedef matrix< _ComponentType, 2 > _Self;
    ...
    public:
        ...
        matrix()
            { std::memset(this->_data, 0, sizeof(component_type) * 2 * 2); }
         ...
         *    ( c2_, c4_ )
         */
        matrix(const component_type & c1_, const component_type & c2_,
               const component_type & c3_, const component_type & c4_)
            {
                this->_data[0][0] = c1_;
                this->_data[0][1] = c2_;
                this->_data[1][0] = c3_;
        ...
        matrix(const component_type * array_)
            { std::memcpy(this->_data, array_, sizeof(component_type) * 2 * 2); }
        ...
        /**  Matrix inverse  */
        component_type inverse()
            {
                return _Self(this->_data[1][1], -this->_data[0][1],
                             -this->_data[1][0], this->_data[0][0]) / this->det();
            }
        ...
        _Self & operator = (const _Self & rhs_)
            {
                std::memcpy(this, &rhs_, sizeof(component_type) * 2 * 2);
                return *this;
            }

        /**  Cast this matrix to a matrix of different element type  */
        template< class _DestinationElementType >
        matrix< _DestinationElementType, 2 > cast() const
            {
                return matrix< _DestinationElementType, 2 >(
        ...
        template< class _DestinationElementType >
        operator matrix< _DestinationElementType, 2 >() const
         ...
         */
        vector< component_type, 2 > row(const size_t & index_)
            {
                // Ensure a sane row
                if (index_ >= 2) {
                    std::cout << "matrix2.h: Requested matrix<2> row index (" << index_ << ") out of bounds" << std::endl;
                    throw std::out_of_range("matrix<2> range check");
                }

                return vector< component_type, 2 >(this->_data[0][index_], this->_data[1][index_]);
            }
         ...
         *  out_of_range if the check fails.
         */
        void row(const size_t & index_, const component_type & x_, const component_type & y_)
            {
                // Ensure a sane row
                if (index_ >= 2) {
                    std::cout << "matrix2.h: Requested matrix<2> row index (" << index_ << ") out of bounds" << std::endl;
         ...
         *  out_of_range if the check fails.
         */
        void row(const size_t & index_, const vector< component_type, 2 > & row_)
            {
                // Ensure a sane row
                if (index_ >= 2) {
                    std::cout << "matrix2.h: Requested matrix<2> row index (" << index_ << ") out of bounds" << std::endl;
         ...
         *  out_of_range if the check fails.
         */
        vector< component_type, 2 > col(const size_t & index_)
            {
                // Ensure a sane column
                if (index_ >= 2) {
                    std::cout << "matrix2.h: Requested matrix<2> column index (" << index_ << ") out of bounds" << std::endl;
                    throw std::out_of_range("matrix<2> range check");
                }

                return vector< component_type, 2 >(this->_data[index_][0], this->_data[index_][1]);
            }
         ...
         *  out_of_range if the check fails.
         */
        void col(const size_t & index_, const component_type & x_, const component_type & y_)
            {
                // Ensure a sane column
                if (index_ >= 2) {
                    std::cout << "matrix2.h: Requested matrix<2> column index (" << index_ << ") out of bounds" << std::endl;
         ...
         *  out_of_range if the check fails.
         */
        void col(const size_t & index_, const vector< component_type, 2 > & col_)
            {
                // Ensure a sane column
                if (index_ >= 2) {
                    std::cout << "matrix2.h: Requested matrix<2> column index (" << index_ << ") out of bounds" << std::endl;
        ...
        component_type & at(const size_t & col_, const size_t & row_)
            {
                ...
                if (row_ >= 2 || col_ >= 2) {
                    std::cout << "matrix2.h: Requested matrix<2> cell index (" << col_ << ", " << row_ << ") out of bounds" << std::endl;
                    throw std::out_of_range("matrix<2> range check");
                }
    ...
    private:
        // Actual matrix data.
        component_type _data[2][2];

    }; /* class Matrix */

} /* namespace gtl */

#endif /* GTL_MATRIX2_INCL_ */
